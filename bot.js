...
$!bce / رسالة جماعية الى كل اعضاء السيرفر بأمبد
${prefix}bc / رسالة جماعيه مع الاختيار بأمبد او بدون
${prefix}rolebc <everyone or @role> / راسال رساله جماعيه لرتبه محدده
${prefix}role @user <rank> / لأعطاء رتبة لعضو معين
${prefix}removerole @user <rank> / لازالة الرتبة من شخص معين
...
${prefix}delete <name> / مسح الشات او الرووم فويس
...
.addField('__Your Name__',`**<@` + `${z.id}` + `>**`, true)
.addField('__ID__', "**"+ `${z.id}` +"**",true)
.addField('__Status__','**'+y+'**' , true)
.addField('__Acount Type__',"**"+ w + "**",true)    
.addField('__Your Tag__',"**#" +  `${z.discriminator}**`,true)
.addField('__Your account created in__' ,year + "-"+ month +"-"+ day)    
.addField("__Entered the server in__", message.member.joinedAt.toLocaleString())    
.addField("__Last Message__", message.author.lastMessage)            
...
message.channel.send(
  ...
  .setColor("#9B59B6")
  .addField(" ** :gear: Server Support :gear: **" , "اسف ليس هناك سيرفر دعم فني")
...
client.users.get("465153571978215434").send(
    "\n" + "**" + "السيرفر :" + "**" +
    "\n" + "**" + "» " + message.guild.name + "**" +
    "\n" + "**" + " المرسل : " + "**" +
    "\n" + "**" + "» " + message.author.tag + "**" +
...
message.react("✅")
 ...
 client.on('message', message => {
  if(!message.channel.guild) return;
if(message.content.startsWith('!bce')) {
if(!message.channel.guild) return message.channel.send('**هذا الأمر فقط للسيرفرات**').then(m => m.delete(5000));
if(!message.member.hasPermission('ADMINISTRATOR')) return      message.channel.send('**للأسف لا تمتلك صلاحية** `ADMINISTRATOR`' );
...
.setDescription('✏️ برودكاست بـ امبد \nبرودكاست بدون امبد 📬 \nلديك دقيقه للأختيار قبل الغاء البرودكاست`)
if (!args) return message.reply('*يجب عليك كتابة كلمة او جملة لإرسال البرودكاست**');message.channel.send(BcList).then(msg => {
msg.react('?')
.then(() => msg.react('✏️'))
.then(() =>msg.react('📬'))
 
let EmbedBcFilter = (reaction, user) => reaction.emoji.name === '✏️' && user.id === message.author.id;
...
client.on('message', message => {
    if(message.content == prefix + 'vip-servers') {
...
e.edit("ارسل مدة الروم بالدقائق بين 2 و 180 دقيقة")
message.channel.awaitMessages(filter, { max: 1, time: 20000, errors: ['time'] })
.then(co => {
...
var Za7f = [
  ...
  "**تاخذ عقابين**.",
  "**قول اسم امك افتخر بأسم امك (مسموح الرفض)**.",
...
client.on('message', async msg => { 
	...
	if (command === `play`) {
		...
		const permissions = voiceChannel.permissionsFor(msg.client.us
...
if (message.author.id !== '434845976050794516') return message.reply('** هذا الأمر فقط لصاحب البوت و شكراًً **')
 ...
 message.channel.sendMessage(`**${argresult}** : تم تغيير الوتشينق الى`)
}
});
client.on('message', async message => {
